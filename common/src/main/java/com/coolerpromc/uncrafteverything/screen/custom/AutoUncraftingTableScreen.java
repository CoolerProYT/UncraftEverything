...
import com.coolerpromc.uncrafteverything.networking.ServerBoundAmountToAddPayload;
import com.coolerpromc.uncrafteverything.networking.ServerBoundExpTransferPayload;
import com.coolerpromc.uncrafteverything.networking.ServerBoundSelectedIndexSyncPayload;
import com.coolerpromc.uncrafteverything.networking.ServerBoundTypeChangePayload;
...
public class AutoUncraftingTableScreen extends AbstractUncraftingScreen<AutoUncraftingTableBlockEntity, AutoUncraftingTableMenu> {
    ...
    private void remove(Button button){
        Services.NETWORK.sendToServer(new ServerBoundExpTransferPayload(this.menu.blockEntity.getBlockPos(), -this.menu.getAmountToAdd(), this.menu.getTypeToAdd()));
    }
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        this.renderExpStored(graphics, mouseX, mouseY);
    }
    ...
    protected void chooseRecipe(RecipeSelectionButton button, int index) {
        ...
        Services.NETWORK.sendToServer(new ServerBoundSelectedIndexSyncPayload(this.menu.blockEntity.getBlockPos(), index));
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollDelta) {
        if (amountWidget.isHovered()){
            if (scrollDelta == 1.0d){
                if (this.menu.getAmountToAdd() < 100){
                    Services.NETWORK.sendToServer(new ServerBoundAmountToAddPayload(this.menu.blockEntity.getBlockPos(), this.menu.getAmountToAdd() + 1));
                }
                else{
                    Services.NETWORK.sendToServer(new ServerBoundAmountToAddPayload(this.menu.blockEntity.getBlockPos(), 1));
                }
            }
                ...
                if (this.menu.getAmountToAdd() > 1){
                    ...
                    Services.NETWORK.sendToServer(new ServerBoundAmountToAddPayload(this.menu.blockEntity.getBlockPos(), 100));
                }
            }
            return true;
        }
        if (typeWidget.isHovered()){
            if (scrollDelta == 1.0d || scrollDelta == -1.0d){
                Services.NETWORK.sendToServer(new ServerBoundTypeChangePayload(this.menu.blockEntity.getBlockPos(), this.menu.getTypeToAdd().invert()));
            }
            return true;
        }
    ...
    protected void extractTooltip(@NotNull GuiGraphicsExtractor guiGraphics, int x, int y) {
        ...
        if (removeExp.isHovered()) {
            ...
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.remove_exp"), maxWidth));
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.level_detail").withStyle(ChatFormatting.GRAY), maxWidth));
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.level_detail_2").withStyle(ChatFormatting.DARK_GRAY), maxWidth));
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.level_detail_3").withStyle(ChatFormatting.DARK_GRAY), maxWidth));
            guiGraphics.setTooltipForNextFrame(font, tooltip, DefaultTooltipPositioner.INSTANCE, x, y, false);
        }

        if (addExp.isHovered()) {
            List<FormattedCharSequence> tooltip = new ArrayList<>();
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.add_exp"), maxWidth));
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.level_detail").withStyle(ChatFormatting.GRAY), maxWidth));
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.level_detail_2").withStyle(ChatFormatting.DARK_GRAY), maxWidth));
            tooltip.addAll(font.split(Component.translatable("screen.uncrafteverything.tooltip.level_detail_3").withStyle(ChatFormatting.DARK_GRAY), maxWidth));
            guiGraphics.setTooltipForNextFrame(font, tooltip, DefaultTooltipPositioner.INSTANCE, x, y, false);
