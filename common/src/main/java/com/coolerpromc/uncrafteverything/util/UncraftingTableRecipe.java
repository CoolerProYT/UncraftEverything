...
public class UncraftingTableRecipe {
    ...
    public boolean contains(Pair<Item, DataComponentPatch> tuple){
        for (ItemStack output : this.outputs){
            if (ItemStack.isSameItemSameComponents(output, new ItemStack(tuple.getLeft().builtInRegistryHolder(), 1, tuple.getRight()))){
                return true;
            }
        }
        return false;
    ...
    public int indexOf(Pair<Item, DataComponentPatch> tuple){
    ...
    public ItemStack getStack(Pair<Item, DataComponentPatch> tuple){
        for (ItemStack output : this.outputs) {
            if (ItemStack.isSameItemSameComponents(output, new ItemStack(tuple.getLeft().builtInRegistryHolder(), 1, tuple.getRight()))) {
                return output;
            }
