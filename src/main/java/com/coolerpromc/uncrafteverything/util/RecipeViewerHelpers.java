...
public class RecipeViewerHelpers {
    public static List<JEIUncraftingTableRecipe> getRecipes(RegistryAccess registryAccess, boolean isREI){
        ...
        Ingredient shulkerBoxIngredient = Ingredient.of(BuiltInRegistries.ITEM.getOrCreateTag(Tags.Items.SHULKER_BOXES).key());
        ...
        if (ClientPayloadHandler.payloadFromServer.allowEnchantedItem()){
            registryAccess.lookup(Registries.ENCHANTMENT)
                    ...
                    .forEach(holder -> {
                        ...
                        book.enchant(holder, enchantment.getMaxLevel());
        ...
        Minecraft.getInstance().level.getRecipeManager().getRecipes().forEach(recipeHolder -> {
            ...
            if (recipeHolder.value() instanceof SmithingTransformRecipe smithingTransformRecipe && ClientPayloadHandler.payloadFromServer.allowUnsmithing()){
                ...
                entries.add(new JEIUncraftingTableRecipe(new ItemStack(smithingTransformRecipe.result.getItem().builtInRegistryHolder(), 1, smithingTransformRecipe.result.getComponentsPatch()), ingredients));
            }

            if (recipeHolder.value() instanceof SmithingTrimRecipe smithingTrimRecipe && ClientPayloadHandler.payloadFromServer.allowUnsmithing()){
