...
public class MultiLineEditBox extends GuiComponent implements GuiEventListener, Widget, NarratableEntry {
    ...
    public MultiLineEditBox(Font fontRenderer, int x, int y, int width, int height, int maxLines) {
    ...
    private void drawScrollbar(PoseStack matrixStack, int visibleLines) {
    ...
    private void handleScrollbarClick(double mouseY) {
        ...
        if (lines.size() > visibleLines) {
            ...
            scrollOffset = Mth.clamp(targetLine, 0, lines.size() - visibleLines);
        }
    }
    ...
    public void scrollToLine(int lineNumber) {
        int visibleLines = getVisibleLines();
        scrollOffset = Mth.clamp(lineNumber - visibleLines / 2, 0, Math.max(0, lines.size() - visibleLines));
    }
