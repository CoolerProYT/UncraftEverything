...
public class MultiLineEditBox extends DrawableHelper implements Element, Drawable {
    ...
    public MultiLineEditBox(TextRenderer fontRenderer, int x, int y, int width, int height, int maxLines) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i < Math.min(visibleLines, lines.size() - scrollOffset); i++) {
            ...
            if (lineIndex < lines.size()) {
                ...
                fontRenderer.drawWithShadow(matrixStack, line, x + 2, y + 2 + i * lineHeight, 0xFFE0E0E0);

                // Draw cursor
                if (isFocused && lineIndex == cursorLine) {
                    String beforeCursor = line.substring(0, Math.min(cursorPos, line.length()));
                    int cursorX = x + 2 + fontRenderer.getWidth(beforeCursor);
                    int cursorY = y + 2 + i * lineHeight;
                    fill(matrixStack, cursorX, cursorY, cursorX + 1, cursorY + fontRenderer.fontHeight, 0xFFE0E0E0);
                }
            }
        }
        ...
        if (wasInBounds && button == 0) {
            ...
            if (clickedLine >= 0 && clickedLine < lines.size()) {
                ...
                for (int i = 0; i <= line.length(); i++) {
                    int charX = fontRenderer.getWidth(line.substring(0, i));
                    if (charX > clickX) {
                        break;
                    }
                    cursorPos = i;
