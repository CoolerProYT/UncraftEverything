...
public class UncraftingTableScreen extends HandledScreen<UncraftingTableMenu> {
    ...
    public void updateFromBlockEntity(List<UncraftingTableRecipe> recipes, int size) {
        ...
        if (size < 7 && this.page != 0){
            ...
            packetByteBuf.encodeAsJson(UncraftingRecipeSelectionDataPayload.CODEC, new UncraftingRecipeSelectionDataPayload(page, this.handler.blockEntity.getPos()));
    ...
    protected void init() {
        ...
        if (this.handler.player.isCreative() || this.handler.player.hasPermissionLevel(4)){
            ButtonWidget configButton = ButtonWidget
                    .builder(Text.translatable("screen.uncrafteverything.blank"), this::openConfigScreen).size(12, 12).position(x + backgroundWidth - 16, y + 3)
                    .build();
    ...
    private void onPressed(ButtonWidget button) {
        PacketByteBuf packetByteBuf = PacketByteBufs.create();
        packetByteBuf.encodeAsJson(UncraftingTableCraftButtonClickPayload.CODEC, new UncraftingTableCraftButtonClickPayload(this.handler.blockEntity.getPos(), hasShiftDown()));
