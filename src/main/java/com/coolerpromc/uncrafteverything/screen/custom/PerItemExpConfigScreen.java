...
public class PerItemExpConfigScreen extends AbstractScrollableScreen {
    ...
    public PerItemExpConfigScreen(Screen parent) {
        super(new TranslationTextComponent("screen.uncrafteverything.per_item_xp_config"), 200);
        this.parent = parent;
    }
    ...
    public void renderBackground(MatrixStack poseStack) {
        ...
        MinecraftForge.EVENT_BUS.post(new GuiScreenEvent.BackgroundDrawnEvent(this, poseStack));
    ...
    public void render(MatrixStack pPoseStack, int mouseX, int mouseY, float delta) {
        ...
        ITextComponent key = new TranslationTextComponent("screen.uncrafteverything.per_item_xp_config.key");
        font.draw(pPoseStack, key, (width / 2 - 115) + (150 - font.width(key)) / 2, (int) (ENTRIES_START_Y - scrollAmount), 0xFFFFFFFF);

        ITextComponent value = new TranslationTextComponent("screen.uncrafteverything.per_item_xp_config.value");
        font.draw(pPoseStack, value, (width / 2 - 115 + 160) + (40 - font.width(value)) / 2, (int) (ENTRIES_START_Y - scrollAmount), 0xFFFFFFFF);

        ITextComponent del = new TranslationTextComponent("screen.uncrafteverything.per_item_xp_config.del");
        ...
        this.children.forEach(renderable -> {
            if (renderable instanceof Button && !scrollableButtons.contains(renderable)) {
                ((Button) renderable).render(pPoseStack, mouseX, mouseY, delta);
            }
        });
    }
