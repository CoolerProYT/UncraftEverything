...
public class UEConfigScreen extends Screen {
    ...
    public void render(@NotNull DrawContext pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        pGuiGraphics.drawTextWrapped(this.textRenderer, format, x, 42 + (this.textRenderer.fontHeight / 2) + 20, textWidth,0xFFFFFF);
        ...
        pGuiGraphics.drawTextWrapped(this.textRenderer, allowEnchantedItem, x, 135 + (this.textRenderer.fontHeight / 2) + 1 - this.textRenderer.getWrappedLinesHeight(allowEnchantedItem, textWidth) / 4, textWidth, 0xFFFFFF);
        pGuiGraphics.drawTextWrapped(this.textRenderer, Text.literal("Experience Type"), x, 158 + (this.textRenderer.fontHeight / 2) + 2, textWidth,0xFFFFFF);
        Text expRequired = Text.literal("Experience Required \n(Level/Point based on previous config)");
        pGuiGraphics.drawTextWrapped(this.textRenderer, expRequired, x, 183 + (this.textRenderer.fontHeight / 2) + 1 - this.textRenderer.getWrappedLinesHeight(expRequired, textWidth) / 4, textWidth,0xFFFFFF);
        pGuiGraphics.drawTextWrapped(this.textRenderer, Text.literal("Allow Unsmithing (Netherite/Trimmed Armor)"), x, 208 + (this.textRenderer.fontHeight / 2) + 2, textWidth, 0xFFFFFF);
    }
