...
public class UEConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        EditBoxWidget restrictionsInput = new EditBoxWidget.Builder().x(x).y(42).build(this.textRenderer, widgetWidth, 88, Text.literal("List of item to restrict for Whitelist/Blacklist"));
    ...
    public void render(@NotNull DrawContext pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        pGuiGraphics.drawTextWithShadow(this.textRenderer, "Restriction Type", x, 18 + (this.textRenderer.fontHeight / 2) + 2, 0xFFFFFFFF);
        Text format = Text.literal("Restricted Items \n(Please write each item in new line)");
        pGuiGraphics.drawWrappedTextWithShadow(this.textRenderer, format, x, 42 + (this.textRenderer.fontHeight / 2) + 20, textWidth,0xFFFFFFFF);
        pGuiGraphics.getMatrices().pushMatrix();
        pGuiGraphics.getMatrices().scale(0.65f, 0.65f);
        pGuiGraphics.getMatrices().translate(x * 1.55f, (42 * 1.6f) + this.textRenderer.getWrappedLinesHeight(format, textWidth) * 2 - (this.textRenderer.fontHeight * 0.65f) + 40);
        ...
        pGuiGraphics.drawWrappedTextWithShadow(this.textRenderer, allowEnchantedItem, x, 135 + (this.textRenderer.fontHeight / 2) + 1 - this.textRenderer.getWrappedLinesHeight(allowEnchantedItem, textWidth) / 4, textWidth, 0xFFFFFFFF);
        pGuiGraphics.drawWrappedTextWithShadow(this.textRenderer, Text.literal("Experience Type"), x, 158 + (this.textRenderer.fontHeight / 2) + 2, textWidth,0xFFFFFFFF);
        Text expRequired = Text.literal("Experience Required \n(Level/Point based on previous config)");
        pGuiGraphics.drawWrappedTextWithShadow(this.textRenderer, expRequired, x, 183 + (this.textRenderer.fontHeight / 2) + 1 - this.textRenderer.getWrappedLinesHeight(expRequired, textWidth) / 4, textWidth,0xFFFFFFFF);
        pGuiGraphics.drawWrappedTextWithShadow(this.textRenderer, Text.literal("Allow Unsmithing (Netherite/Trimmed Armor)"), x, 208 + (this.textRenderer.fontHeight / 2) + 2, textWidth, 0xFFFFFFFF);
    }
