...
public class AutoUncraftingTableScreen extends AbstractUncraftingScreen<AutoUncraftingTableBlockEntity, AutoUncraftingTableMenu> {
    ...
    private void remove(Button button){
        ClientPlayNetworking.send(new ExpTransferPayload(this.menu.blockEntity.getBlockPos(), -this.menu.getAmountToAdd(), this.menu.getTypeToAdd()));
    }
    ...
    protected void chooseRecipe(RecipeSelectionButton button, int index) {
        ...
        ClientPlayNetworking.send(new SelectedIndexSyncPayload(this.menu.blockEntity.getBlockPos(), index));
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollDelta) {
        if (amountWidget.isHovered()){
            if (scrollDelta == 1.0d){
                if (this.menu.getAmountToAdd() < 100){
                    ClientPlayNetworking.send(new AmountToAddPayload(this.menu.blockEntity.getBlockPos(), this.menu.getAmountToAdd() + 1));
                }
                else{
                    ClientPlayNetworking.send(new AmountToAddPayload(this.menu.blockEntity.getBlockPos(), 1));
                }
            }
                ...
                if (this.menu.getAmountToAdd() > 1){
                    ...
                    ClientPlayNetworking.send(new AmountToAddPayload(this.menu.blockEntity.getBlockPos(), 100));
                }
            }
            return true;
        }
        if (typeWidget.isHovered()){
            if (scrollDelta == 1.0d || scrollDelta == -1.0d){
                ClientPlayNetworking.send(new TypeChangePayload(this.menu.blockEntity.getBlockPos(), this.menu.getTypeToAdd().invert()));
            }
            return true;
        }
