...
public class FTBQuestsProgressionConfigScreen extends AbstractScrollableScreen {
    ...
    public FTBQuestsProgressionConfigScreen(Screen parent) {
        super(new TranslatableText("screen.uncrafteverything.ftb_quest_progression_config"), 200);
        this.parent = parent;
    }
    ...
    private void saveButtonPressed(ButtonWidget button){
        ...
        ClientPlayNetworking.send(UEProgressionPayload.TYPE, UEProgressionPayload.encode(PacketByteBufs.create(), configPayload));
    ...
    @Override
    public void render(@NotNull MatrixStack guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        GL11.glScissor(0, (windowHeight - 5 - ENTRIES_END_Y * scale), this.width * scale, (ENTRIES_END_Y + 5 - ENTRIES_START_Y) * scale);

        TranslatableText key = new TranslatableText("screen.uncrafteverything.per_item_xp_config.key");
        textRenderer.draw(guiGraphics, key, (width / 2 - 170) + (150 - textRenderer.getWidth(key)) / 2, (int) (ENTRIES_START_Y - scrollAmount), 0xFFFFFFFF);

        TranslatableText value = new TranslatableText("screen.uncrafteverything.ftb_quest_progression_config.value");
        textRenderer.draw(guiGraphics, value, (width / 2 - 170 + 160) + (150 - textRenderer.getWidth(value)) / 2, (int) (ENTRIES_START_Y - scrollAmount), 0xFFFFFFFF);

        TranslatableText del = new TranslatableText("screen.uncrafteverything.per_item_xp_config.del");
        textRenderer.draw(guiGraphics, del, (width / 2 - 170 + 320) + (20 - textRenderer.getWidth(del)) / 2, (int) (ENTRIES_START_Y - scrollAmount), 0xFFFFFFFF);
    ...
    private class Entry {
        ...
        void initWidgets(int x, int y) {
            ...
            valueBox = new TextFieldWidget(textRenderer, x + 160, y, 150, 20, new TranslatableText("screen.uncrafteverything.value"));
            valueBox.setText(currentValue);

            deleteButton = new ButtonWidget(x + 320, y, 20, 20, new TranslatableText("screen.uncrafteverything.x"), b -> {
                entries.remove(this);
                init();
            });
        }
