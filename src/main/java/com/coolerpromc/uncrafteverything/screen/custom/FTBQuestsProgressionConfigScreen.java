...
public class FTBQuestsProgressionConfigScreen extends AbstractScrollableScreen {
    ...
    protected void init() {
        ...
        addButton = new Button(width / 2 - 100, height - 53, 200, 20, Component.translatable("screen.uncrafteverything.add_new_entry"), b -> {
            entries.add(new Entry("", ""));
            this.init();
        });
        addRenderableWidget(addButton);

        cancelButton = new Button(width / 2 - width / 3 - 10, height - 28, width / 3, 20, Component.translatable("screen.uncrafteverything.cancel"), button -> onClose());
        addRenderableWidget(cancelButton);

        saveButton = new Button(width / 2 + 10, height - 28, width / 3, 20, Component.translatable("screen.uncrafteverything.save"), this::saveButtonPressed);
    ...
    public void render(@NotNull PoseStack guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        enableScissor(0, ENTRIES_START_Y - 5, this.width, ENTRIES_END_Y);
    ...
    private class Entry {
        ...
        void initWidgets(int x, int y) {
            ...
            deleteButton = new Button(x + 320, y, 20, 20, Component.translatable("screen.uncrafteverything.x"), b -> {
                entries.remove(this);
                init();
            });
        }
