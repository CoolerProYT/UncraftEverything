...
public class FTBQuestsProgressionConfigScreen extends AbstractScrollableScreen {
    ...
    public FTBQuestsProgressionConfigScreen(Screen parent) {
        super(new TranslationTextComponent("screen.uncrafteverything.ftb_quest_progression_config"), 200);
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        addButton = new Button(width / 2 - 100, height - 53, 200, 20, new TranslationTextComponent("screen.uncrafteverything.add_new_entry"), b -> {
            entries.add(new Entry("", ""));
            this.init();
        });
        addWidget(addButton);

        cancelButton = new Button(width / 2 - width / 3 - 10, height - 28, width / 3, 20, new TranslationTextComponent("screen.uncrafteverything.cancel"), button -> onClose());
        addWidget(cancelButton);

        saveButton = new Button(width / 2 + 10, height - 28, width / 3, 20, new TranslationTextComponent("screen.uncrafteverything.save"), this::saveButtonPressed);
    ...
    public void renderBackground(MatrixStack guiGraphics) {
        ...
        MinecraftForge.EVENT_BUS.post(new GuiScreenEvent.BackgroundDrawnEvent(this, guiGraphics));
    ...
    public void render(MatrixStack guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        TranslationTextComponent key = new TranslationTextComponent("screen.uncrafteverything.per_item_xp_config.key");
        font.draw(guiGraphics, key, (width / 2 - 170) + (150 - font.width(key)) / 2, (int) (ENTRIES_START_Y - scrollAmount), 0xFFFFFFFF);

        TranslationTextComponent value = new TranslationTextComponent("screen.uncrafteverything.ftb_quest_progression_config.value");
        font.draw(guiGraphics, value, (width / 2 - 170 + 160) + (150 - font.width(value)) / 2, (int) (ENTRIES_START_Y - scrollAmount), 0xFFFFFFFF);

        TranslationTextComponent del = new TranslationTextComponent("screen.uncrafteverything.per_item_xp_config.del");
    ...
    protected void renderSeparator(MatrixStack poseStack){
    ...
    private class Entry {
        ...
        void initWidgets(int x, int y) {
            ...
            valueBox = new TextFieldWidget(font, x + 160, y, 150, 20, new TranslationTextComponent("screen.uncrafteverything.value"));
            valueBox.setValue(currentValue);

            deleteButton = new Button(x + 320, y, 20, 20, new TranslationTextComponent("screen.uncrafteverything.x"), b -> {
                entries.remove(this);
                init();
            });
        }
