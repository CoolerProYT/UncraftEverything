...
public class UncraftingTableBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory<BlockPos> {
    ...
    private final ImplementedInventory slots = new ImplementedInventory(10){
        ...
        public void setStack(int slot, ItemStack stack) {
            ...
            if (world != null && !world.isClient() && slot == inputSlots[0] && player != null) {
                ...
                ServerPlayNetworking.send(player, new UncraftingTableDataPayload(getPos(), new ArrayList<>(currentRecipes.subList(currentRecipes.isEmpty() ? 0 : page * 7, Math.min(page * 7 + 7, currentRecipes.size()))), currentRecipes.size()));
    ...
    public void getOutputStacks() {
        ...
        List<RecipeEntry<?>> recipes = serverLevel.getRecipeManager().values().stream().filter(recipeHolder -> {
            if (!recipeHolder.id().getNamespace().equals("minecraft") && Registries.ITEM.getId(inputStack.getItem()).getNamespace().equals("minecraft") && UncraftEverythingConfig.preventModdedIngredientRecipes()){
                return false;
            }
            ...
            if (recipeHolder.value() instanceof SmithingTrimRecipe smithingTrimRecipe){
                ...
                if (armorTrim != null){
                    ...
                    Optional<RegistryEntry.Reference<ArmorTrimPattern>> trimPatternReference = ArmorTrimPatterns.get(this.world.getRegistryManager(), Arrays.stream(smithingTrimRecipe.template.getMatchingStacks()).toList().getFirst());
                    if (ingredient != Ingredient.EMPTY && trimPatternReference.isPresent() && armorTrim.getPattern().equals(trimPatternReference.get())){
                        return true;
                    }
                }
            }
        ...
        for (RecipeEntry<?> r : recipes) {
            ...
            if (r.value() instanceof SmithingTrimRecipe smithingTrimRecipe){
                ...
                Arrays.stream(smithingTrimRecipe.addition.getMatchingStacks()).filter(itemStack -> {
                    if (armorTrim != null){
                        RegistryKey<ArmorTrimMaterial> armorTrimKey = armorTrim.getMaterial().getKey().orElse(null);
                ...
                for (List<Item> ingredientCombination : allIngredientCombinations) {
                    ...
                    for (Item item : ingredientCombination) {
                        if (outputStack.getOutputs().contains(item.getDefaultStack())) {
                            ItemStack stack = outputStack.getOutputs().get(outputStack.getOutputs().indexOf(item.getDefaultStack()));
                            if (item.getDefaultStack().isOf(Arrays.stream(ingredients.getFirst().getMatchingStacks()).toList().getFirst().getItem())){
    ...
    public static boolean isVanillaIngredientRecipe(Recipe<?> recipe) {
        ...
        for (Ingredient ingredient : ingredients) {
            if (!Arrays.stream(ingredient.getMatchingStacks()).map(ItemStack::getItem).map(Registries.ITEM::getId).map(Identifier::getNamespace).toList().contains("minecraft")) {
                return false;
            }
        }
