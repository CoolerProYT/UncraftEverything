...
public class UncraftingTableBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory {
    ...
    private final ImplementedInventory slots = new ImplementedInventory(10){
        ...
        public void setStack(int slot, ItemStack stack) {
            ...
            if (world != null && !world.isClient() && slot == inputSlots[0] && player != null) {
                ...
                try{
                    packetByteBuf.encode(UncraftingTableDataPayload.CODEC, new UncraftingTableDataPayload(getPos(), new ArrayList<>(currentRecipes.subList(currentRecipes.isEmpty() ? 0 : page * 7, Math.min(page * 7 + 7, currentRecipes.size()))), currentRecipes.size()));
                } catch (IOException e) {
    ...
    public Identifier inputStackLocation() {
        return Registry.ITEM.getId(slots.getStack(inputSlots[0]).getItem());
    }

    public void getOutputStacks() {
        if (!(world instanceof ServerWorld) || player == null) return;
        ...
        if (slots.getStack(inputSlots[0]).isEmpty()
                ...
                || (!UncraftEverythingConfig.isEnchantedItemsAllowed(slots.getStack(inputSlots[0])) && inputStack.getTag() != null && !inputStack.getTag().contains("Trim"))
            ...
            if (!UncraftEverythingConfig.isEnchantedItemsAllowed(slots.getStack(inputSlots[0])) && inputStack.getTag() != null && !inputStack.getTag().contains("Trim")){
        ...
        List<Recipe<?>> recipes = serverLevel.getRecipeManager().values().stream().filter(recipeHolder -> {
            if (!recipeHolder.getId().getNamespace().equals("minecraft") && Registry.ITEM.getId(inputStack.getItem()).getNamespace().equals("minecraft") && UncraftEverythingConfig.preventModdedIngredientRecipes()){
                return false;
            }
        ...
        if (!recipes.isEmpty() || inputStack.getItem().equals(Items.TIPPED_ARROW) || (UncraftEverythingConfig.allowEnchantedItems && !EnchantmentHelper.get(inputStack).isEmpty() && !inputStack.getItem().equals(Items.ENCHANTED_BOOK))){
            this.status = -1;
            ...
            if (r instanceof ShapedRecipe) {
                ...
                for (List<Item> ingredientCombination : allIngredientCombinations) {
                    ...
                    if (inputStack.isDamaged()){
                        for (Map.Entry<Item, Integer> x : allIngredients.entrySet()){
                            if (inputStack.getItem().canRepair(inputStack, new ItemStack(x.getKey(), x.getValue()))){
                                ...
                                for (int i = 0;i < outputStack.getOutputs().size() && damagedPercentage != 0;i++){
                                    if (outputStack.getOutputs().get(i).getItem().equals(x.getKey())){
    ...
    private List<List<Item>> getAllIngredientCombinations(DefaultedList<Ingredient> ingredients) {
        ...
        for (int i = 0; i < ingredients.size(); i++) {
            ...
            items = items.stream().filter(item -> {
                boolean isVanillaInput = Registry.ITEM.getId(slots.getStack(0).getItem()).getNamespace().equals("minecraft");
    ...
    private int getExperience() {
        ...
        for (Map.Entry<String, Integer> exp : experienceMap.entrySet()){
            if (exp.getKey().startsWith("#")){
                ...
                if (tagKey.isPresent() && slots.getStack(inputSlots[0]).getItem().isIn(tagKey.get())) {
